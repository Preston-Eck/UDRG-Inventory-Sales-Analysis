...
const SettingsModal = ({ settings, onSave, onClose, onMigrate }: { settings: AppSettings, onSave: (s: AppSettings) => void, onClose: () => void, onMigrate: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[100] p-4 font-sans text-sm">
      <div className="bg-[var(--card-bg)] rounded-lg w-full max-w-2xl border border-[var(--border-color)] shadow-2xl flex flex-col max-h-[90vh] text-[var(--text-color)]">
        ...
        {/* Data Management */}
        <section>
          <h4 className="text-xs uppercase font-bold text-slate-500 mb-4 border-b border-[var(--border-color)] pb-1">Data Management</h4>
          <div className="bg-[var(--app-bg)] p-4 rounded border border-[var(--border-color)] flex justify-between items-center">
            <div>
              <h5 className="font-bold text-sm">Supabase Migration</h5>
              <p className="text-xs text-[var(--text-muted)]">Upload current locally loaded data to the connected Supabase database.</p>
            </div>
            <Button onClick={() => { onClose(); onMigrate(); }} variant="secondary" className="border-indigo-500 text-indigo-400 hover:bg-indigo-900/20">
              <i className="fa-solid fa-cloud-arrow-up mr-2"></i> Migrate Now
            </Button>
          </div>
        </section>
      </div>
...
const App = () => {
    ...
    try {
      ...
      if (conn.success && (conn.count || 0) > 0) {
        ...
        const sbProducts: Product[] = resProducts.data.map(p => ({
          sku: p.sku, name: p.name, department: p.department, category: p.category,
          vendor: p.vendor, cost: p.cost, price: p.price
        }));

        const sbInventory: InventoryState[] = resInventory.data.map(i => ({
          sku: i.sku, qtyOnHand: i.qty_on_hand
        }));

        const sbTransactions: Transaction[] = resTransactions.data.map(t => ({
          id: t.id || Math.random().toString(), // Handle legacy text ids
          date: t.date,
          sku: t.sku,
          qtySold: t.qty_sold,
          discount: t.discount,
      ...
      console.warn("Supabase Fetch Failed (or empty), falling back to GAS/Mock", e);
    }

    // Fallback to GAS (Existing Logic)
  ...
  // --- Optimized Data Indices ---

  const { transactionIndex, historyIndex, propertyIndex, groupedTx } = useMemo(() => {
    const txIndex = new Map<string, Transaction[]>();
    const histIndex = new Map<string, number>();
    const propIndex = new Map<string, Transaction[]>();

    // Global filter first to avoid processing irrelevant data if possible
    // But since we need dynamic filtering, we index ALL transactions once.

    transactions.forEach(t => {
      // Index by SKU
      if (!txIndex.has(t.sku)) txIndex.set(t.sku, []);
      txIndex.get(t.sku)!.push(t);

      // Index by Property (for Property filter)
      if (!propIndex.has(t.property)) propIndex.set(t.property, []);
      propIndex.get(t.property)!.push(t);

      // Index by History (SKU + Month + Year) for fast calendar lookups
      // Format: SKU_YYYY_M
      const d = new Date(t.date);
      const key = `${t.sku}_${d.getFullYear()}_${d.getMonth()}`;
      histIndex.set(key, (histIndex.get(key) || 0) + t.qtySold);
    });

    return { transactionIndex: txIndex, historyIndex: histIndex, propertyIndex: propIndex, groupedTx: transactions };
  }, [transactions]);
  ...
  const analyzedData: AnalysisRow[] = useMemo(() => {
    ...
    // 1. Filter Transactions by Property & Date Range ONCE
    const relevantTxMap = new Map<string, Transaction[]>(); // SKU -> Filtered Tx
    const relevantHistoryMap = new Map<string, number>(); // SKU_YYYY_M -> Qty (Filtered by Property)

    const dateStartObj = new Date(filters.dateStart);
    const dateEndObj = new Date(filters.dateEnd);

    // Optimization: If "All" properties, use the master index, else use property index
    const sourceTx = filters.selectedProperty === 'All' ? transactions : (propertyIndex.get(filters.selectedProperty) || []);

    // We still have to iterate sourceTx to filter by DATE, but we can build a mini-index for the current view
    sourceTx.forEach(t => {
      if (t.date >= filters.dateStart && t.date <= filters.dateEnd) {
        if (!relevantTxMap.has(t.sku)) relevantTxMap.set(t.sku, []);
        relevantTxMap.get(t.sku)!.push(t);
      }

      // For history, we need the FULL date range, not just the selected range, 
      // BUT we must respect the Property filter.
      const d = new Date(t.date);
      const key = `${t.sku}_${d.getFullYear()}_${d.getMonth()}`;
      relevantHistoryMap.set(key, (relevantHistoryMap.get(key) || 0) + t.qtySold);
    });
    ...
    const calculateMetrics = (skus: string[], id: string, name: string, category: string, isGroup: boolean): AnalysisRow => {
      ...
      skus.forEach(sku => {
        ...
        // FAST LOOKUP using our relevantTxMap
        const skuTx = relevantTxMap.get(sku) || [];
      ...
      for (let i = 1; i <= 12; i++) {
        ...
        for (let y = 1; y <= 3; y++) {
          ...
          skus.forEach(sku => {
            // INSTANT LOOKUP for history
            const key = `${sku}_${lookbackYear}_${targetMonth}`;
            const sales = relevantHistoryMap.get(key) || 0;
        ...
        // Fallback if no history: Use current average demand
        const fallbackDemand = qtySold / 12; // Very rough approximation
        const forecastedDemand = historicalAverage > 0 ? Math.ceil(historicalAverage * 1.05) : Math.ceil(fallbackDemand);
    ...
    return result;
  }, [products, transactions, inventory, customGroups, filters, transactionIndex, historyIndex, propertyIndex]); // Depend on indices
  ...
  const handleSendMessage = async (customPrompt?: string) => {
    ...
    // Naive keyword matching to find relevant products to feed the AI
    let contextData = "";
    ...
    if (detailModal && text.includes("EXPLAIN_CELL")) {
      ...
      // 1. Find relevant products by keyword
      const mentionedRows = analyzedData.filter(r =>
        lowerText.includes(r.name.toLowerCase()) ||
        lowerText.includes(r.category.toLowerCase()) ||
        lowerText.includes(r.department.toLowerCase())
      ).slice(0, 10); // Limit to 10 to fit context window

      if (mentionedRows.length > 0) {
        contextData += "RELEVANT ITEMS FOUND:\n";
        mentionedRows.forEach(r => {
          contextData += `- ${r.name}: OnHand=${r.qtyOnHand}, Sold=${r.qtySold}, Profit=$${r.profit.toFixed(0)}, ReorderSuggestion=${r.suggestedReorder}\n`;
        });
      } else {
        // If no specific items found, give top movers or overview
        contextData += "TOP MOVERS (Revenue):\n";
        analyzedData.slice(0, 5).forEach(r => {
          contextData += `- ${r.name}: Revenue=$${r.revenue.toFixed(0)}\n`;
        });
      }

      // 2. Global Totals
      const totalRev = analyzedData.reduce((a, b) => a + b.revenue, 0);
      const totalProfit = analyzedData.reduce((a, b) => a + b.profit, 0);
      const itemsReorder = analyzedData.filter(r => r.suggestedReorder > 0).length;

      contextData += `\nGLOBAL STATS:\n- Total Revenue: $${totalRev.toLocaleString()}\n- Total Profit: $${totalProfit.toLocaleString()}\n- Items Needing Restock: ${itemsReorder}`;

      finalPrompt = `
           User Question: "${text}"
           
           APP CONTEXT:
           The user is looking at a Dashboard of inventory.
           Date Range: ${filters.dateStart} to ${filters.dateEnd}
           Property: ${filters.selectedProperty}
           
           DATA CONTEXT (Use this to answer!):
           ${contextData}
           
           INSTRUCTIONS:
           - Answer the user's question directly using the data provided above.
           - If the exact answer isn't in the data, give the best estimate based on the "Relavant Items" or "Global Stats".
    ...
    if (window.google && window.google.script) {
      ...
      setTimeout(() => {
        setMessages(prev => [...prev, { role: 'model', text: `[Local Dev] I received your request.\nContext Data:\n${contextData}`, timestamp: new Date() }]);
        setIsThinking(false);
      }, 1000);
    }
  };
    ...
    <div className={`min-h-screen flex flex-col md:flex-row bg-[var(--app-bg)] text-[var(--text-color)] font-sans ${settings.fontSize}`}>
      ...
      <aside className="w-full md:w-64 bg-[var(--sidebar-bg)] border-r border-[var(--border-color)] flex flex-col h-screen flex-shrink-0 z-20 transition-colors">
        ...
        <div className="flex-1 overflow-y-auto px-4 space-y-6">
          ...
          <div>
            <label className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-2 block">Reporting Period</label>
            <div className="grid grid-cols-2 gap-2 mb-2">
              <button onClick={() => {
                const e = new Date();
                const s = new Date(); s.setDate(e.getDate() - 30);
                setFilters(p => ({ ...p, dateStart: s.toISOString().split('T')[0], dateEnd: e.toISOString().split('T')[0] }));
              }} className="text-[10px] bg-[var(--card-bg)] border border-[var(--border-color)] py-1 rounded text-[var(--text-muted)] hover:text-white hover:border-[var(--primary-color)] transition-colors">Last 30 Days</button>
              <button onClick={() => {
                const e = new Date();
                const s = new Date(new Date().getFullYear(), 0, 1);
                setFilters(p => ({ ...p, dateStart: s.toISOString().split('T')[0], dateEnd: e.toISOString().split('T')[0] }));
              }} className="text-[10px] bg-[var(--card-bg)] border border-[var(--border-color)] py-1 rounded text-[var(--text-muted)] hover:text-white hover:border-[var(--primary-color)] transition-colors">YTD (This Year)</button>
              <button onClick={() => {
                const y = new Date().getFullYear() - 1;
                setFilters(p => ({ ...p, dateStart: `${y}-01-01`, dateEnd: `${y}-12-31` }));
              }} className="text-[10px] bg-[var(--card-bg)] border border-[var(--border-color)] py-1 rounded text-[var(--text-muted)] hover:text-white hover:border-[var(--primary-color)] transition-colors">Last Year</button>
              <button onClick={() => {
                const s = new Date(); s.setMonth(s.getMonth() - 12);
                const e = new Date();
                setFilters(p => ({ ...p, dateStart: s.toISOString().split('T')[0], dateEnd: e.toISOString().split('T')[0] }));
              }} className="text-[10px] bg-[var(--card-bg)] border border-[var(--border-color)] py-1 rounded text-[var(--text-muted)] hover:text-white hover:border-[var(--primary-color)] transition-colors">Last 12 Mos</button>
            </div>
            <div className="flex items-center gap-2">
              <input
                type="date"
                ...
                className="w-full bg-[var(--card-bg)] text-[10px] border border-[var(--border-color)] rounded p-1 outline-none text-[var(--text-color)]"
              />
      ...
      {isMigrationOpen && (
        <MigrationModal
          data={{ products, transactions, inventory }}
